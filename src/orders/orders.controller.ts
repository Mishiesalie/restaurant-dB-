...
import { orderService, getordersService, createordersService, updateordersService, deleteordersService } from "./orders.service";
...
export const createorders = async (c: Context) => {
    try {
        ...
        const createdorders = await createordersService(user);
...
export const updateorders = async (c: Context) => {
    ...
    try {
        // search for the user
        const searchedUser = await getordersService(id);
        if (searchedUser == undefined) return c.text("User not found", 404);
        // get the data and update it
        const res = await updateordersService(id, user);
...
export const deleteorders = async (c: Context) => {
    ...
    try {
        ...
        const user = await getordersService(id);
        if (user == undefined) return c.text("orders not found", 404);
        //deleting the orders
